...
Wiring it yourself skips the theme, so the tab bar falls back to platform defaults. Pass `defaultTintColors` to supply your own fallbacks, either as a prop or from a mapper:

```tsx
createStandardNavigationFactories(nativeBottomTabsNavigator, TabRouter, () => {
